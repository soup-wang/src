...
# 引入ros的Python包
import rospy
from mavros_msgs.msg import State, OverrideRCIn, RCIn, RCOut, VFR_HUD, HomePosition, GPSRAW
...
class Px4Controller:
    ...
    def __init__(self):
        ...
        self.speed_is_small = False
        self.speed_is_small_start_time = 0
        self.keep_yaw = 0

        self.bridge_origin_time = 0
        self.bridge_parallel_yaw = 0
        self.yaw = 0.0    # 偏航角度
        ...
        #  gps 状态
        rospy.Subscriber("/mavros/gpsstatus/gps1/raw", GPSRAW, self.GPSRAW_callback)
    ...
    def start(self, debug=False):
        ...
        while True:
            ...
            # self.log.logger.info("输入"+str(self.rc_in))
            # 当2通道小于1000进入失控保护
            # if self.rc_in[1] < 1000:
                # rcmsg
            if self.arm_state and self.manual_state and (rospy.is_shutdown() is False):
                ...
                if self.current_gps != None and len(self.rc_in) > 2 and self.rc_in[5] > 1500:
                    ...
                    if bridge_status == 3:
                        ...
                        if self.speed_is_small and time.time() - self.speed_is_small_start_time >240:
                            # 动力阻塞倒退
                            for i in range(50):
                                rcmsg.channels[2] = 1350
                            ...
                            self.speed_is_small = False
                            self.speed_is_small_start_time = time.time()
                            rcmsg.channels[2] = 1750
                        ...
                        if len(min_bridge_waypoints) > 0:
                            ...
                            self.bridge_parallel_yaw  = bridge_heading
                            ...
                            if len(self.all_points) > 130 :
                                ...
                                if self.left_dist != None and self.left_parallel_yaw != None and self.left_dist > 0.5:
                                    ...
                                    self.set_dist_pid_control(
                                        self.config.except_dist, self.left_dist, self.left_parallel_yaw, rcmsg, self.current_heading , self.config.course_range , hum_msg)
                                else:
                                    self.left_dist = None 
                                self.left_parallel_yaw = None
                            else:
                    ...
                    # 若当前位置距离桥第一个点小于最小航点距离则进行
                        ...
                        if current_bridge_distance < 8:
                            
                            bridge_status = 1
                            start_time = time.time()
                            self.bridge_origin_time = (bridge_distance -10) / self.config.ground_speed
                            self.log.logger.info(
                                "到达桥起始点" +  str(self.current_gps) + "通过时间:"+str(self.bridge_origin_time)+"速度:"+str(self.config.ground_speed))

                    elif bridge_status == 1:
                        
                        # self.get_dist_parallel_yaw()
                        #     # 判断点云是否符合沿岸要求，符合要求是时进行沿岸行走,不符合要求航向向下个航点行驶,或者进入调试模式下不进入延边模式
                        # if len(self.all_points) > 5 :
                        #     self.get_line_slope()
                        #     if self.left_dist != None and self.left_parallel_yaw != None and self.left_dist > 0.1:
                        #         hum_msg.leftDist = self.left_dist
                        #         hum_msg.leftParallelYaw = self.left_parallel_yaw
                        #         if len(self.all_points) > 30:
                        #            self.bridge_parallel_yaw = self.left_parallel_yaw 

                        #         # 根据与岸的距离不断调整 yaw 是其不断逼近想要的距离
                        #         self.set_dist_pid_control(
                        #             9, self.left_dist, self.bridge_parallel_yaw, rcmsg, bridge_heading ,15,hum_msg)
                        #     else:
                        #         self.left_dist = None 
                        #     self.left_parallel_yaw = None
                        # else:
...
# bridge_time > self.bridge_origin_time or


                        if bridge_time > self.bridge_origin_time and self.gps_hdop < 1:
                            # 等待10秒等待卫星定位计算定位与当前位置的差
                            self.wait_gps(15,rcmsg)
                            ...
                            diff_yaw = angle_utils.angle_min_different_0_360(heading,bridge_heading)
                            self.log.logger.info("角度:"+str(diff_yaw))
                            if   (dist < 10 or diff_yaw >90) and self.gps_hdop < 1:
                                self.log.logger.info(
                                ...
                                # 确定通过桥梁
                                # 过桥后等待10秒 gps重新定位 
                                # self.wait_gps(10,rcmsg)
                                rcmsg.channels[2] = 1750
                                bridge_status = 2
                            else:
                                self.bridge_origin_time = 10
                                start_time = time.time()
                                rcmsg.channels[2] = 1750
    ...
    def get_min_bridge(self):
        ...
        if bridge_heading == None or angle_utils.angle_min_different_0_360(bridge_heading, self.yaw) > 75:
            min_bridge_waypoints = []
    ...
    def set_dist_pid_control(self, except_dist, now_dist, parallel_yaw, rcmsg ,current_heading ,course_range,hum_msg):
        ...
        # self.log.logger.info("parallel_yaw:"+str(parallel_yaw))
        # self.log.logger.info("except_yaw:"+str(except_yaw))

        # 期望值yaw 在执行任务时最大值不得与当前 位置到航点位置的方位角相差30度
        except_yaw = self.except_yaw_range(except_yaw,current_heading ,course_range)
        # print ("last_except_yaw", except_yaw)
        
        # 根据期望yaw 值不断调整车身与岸距离
        self.set_yaw_pid_control(except_yaw, rcmsg ,hum_msg)

    def except_yaw_range(self, except_yaw ,current_heading ,course_range):
        '''
        根据期望角度范围设置期望角度
        '''
       
        # 判断是否在区间范围内
        if angle_utils.angle_min_different_0_360(current_heading,except_yaw) > course_range :
            current_heading_up = (current_heading + course_range) %360
            current_heading_down = (current_heading - course_range) %360
            # 计算期望角度与哪个边界角度近取哪个角度
...
# imu 回调函数
    def imu_callback(self, msg):
        ...
        # s = math.sqrt(self.imu.linear_acceleration.x **2 + self.imu.linear_acceleration.y **2 + self.imu.linear_acceleration.z **2) 
        # self.log.logger.info("速度"+str(s))
    ...
    def velocity_callback(self , msg):
        ...
        ground_speed = math.sqrt(linear.x **2 + linear.y **2)-0.15
        avg_speed = 0
        if time.time() -self.speed_start_time <15 and angle_utils.angle_min_different_0_360(self.yaw,self.keep_yaw) <2:
            self.speed_list.append(ground_speed) 
            avg_speed = sum(self.speed_list)/len(self.speed_list)
        else:
            self.keep_yaw = self.yaw
            self.speed_start_time = time.time()
            self.speed_list = []
        
        if 0.37 > avg_speed > 0.3:
            self.config.ground_speed = avg_speed
        elif 0.30 >avg_speed >0 and self.rc_in[2] > 1500:
            #直行速度过低倒退停止一下
            self.speed_is_small = True
        elif avg_speed < 0:
            self.speed_is_small = None

        else: 
            self.config.ground_speed = 0.36

        # 若直行速度过慢需要 
        # elif 

        # self.log.logger.info("平均速度:"+str(avg_speed))
    ...
    def GPSRAW_callback(self,msg):
        '''
        获取 gps 信息状态
        eph: gps 定位精度位置因子 hdop
        satellites_visible : gps 定位卫星数量
        '''
        self.gps_hdop = msg.eph / 100.0
        self.gps_vdop = msg.epv / 100.0

        self.gps_hdop  = self.max_value(self.gps_hdop,self.gps_hdop_queue,6 )
        self.satellites_visible = msg.satellites_visible
        # self.log.logger.info("定位精度:"+str(self.gps_hdop))
        # self.log.logger.info("定位精度:"+str(self.gps_vdop))
        # self.log.logger.info("定位精度:"+str(self.gps_vdop **2 + self.gps_hdop **2 ))

        # self.log.logger.info("卫星数量:"+str(self.satellites_visible))

    '''
    return yaw from current IMU
    '''
    def max_value(self,value ,max_queue,queue_len = 5):
        '''
        获取一系列数中的最大值
        '''
        # Queue().qsize
        max_queue.put(value)
        if max_queue.qsize() == queue_len:
            value = max(max_queue.queue)
            max_queue.get()
